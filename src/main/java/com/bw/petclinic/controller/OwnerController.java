...
public class OwnerController {
    ...
    @GetMapping("/owners")
    public String listOwners(@RequestParam(value = "pageNumber", defaultValue = "1") int pageNumber,
                             @RequestParam(value = "pageSize", defaultValue = "5") int pageSize,
                             Owner owner, BindingResult bindingResult, Model model) {
        log.info("GET listOwners pageNumber [{}], pageSize [{}], owner {}", pageNumber, pageSize, owner);
        Page<Owner> ownerPage = ownerService.findAll(pageNumber - 1, pageSize, owner.getLastName());
        if (ownerPage.getContent().isEmpty()) {
            bindingResult.addError(new ObjectError("lastName", "Owner not found for lastName [" + owner.getLastName() + "]"));
            return "ownerFind";
        }
